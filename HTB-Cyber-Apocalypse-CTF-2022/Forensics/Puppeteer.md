...
The first thing we should do is analyze all the Log files to see if we can catch any useful information. To do that we will use the [Event Viewer](https://docs.microsoft.com/en-us/shows/inside/event-viewer "Event Viewer") app on Windows.

In the `Microsoft-Windows-PowerShell%4Operational.evtx` file and after going through the warnings we find a powershell script that is being created to run some code.

![puppeteer1](./img/Puppeteer/puppeteer1.png)
...
![puppeteer2](./img/Puppeteer/puppeteer2.png)
